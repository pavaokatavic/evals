...
Using OpenAI GPT-5.1 with Responses API and function calling

PROS:
- Strict schema enforcement (enum guarantees only "pass" or "fail")
- No text parsing needed - structured JSON output
- Consistent results (no position bias in comparisons)
- Responses API supports chain-of-thought between turns

CONS:
- Pay-per-use (no free tier)
- More complex setup (tool definitions)
- API differs from Chat Completions (input vs messages, output_text vs choices)
"""
...
# Function calling tools for reliable structured output
EVAL_TOOL = {
    "type": "function",
    ...
    "parameters": {
        ...
        "properties": {
            "verdict": {
                "type": "string",
                "enum": ["pass", "fail"],
                "description": "pass if correct/helpful, fail if wrong/unhelpful"
            }
        },
        "required": ["verdict"],
...
COMPARE_TOOL = {
    ...
    "description": "Submit which response is better",
    "strict": True,
    "parameters": {
        "type": "object",
        "properties": {
            "winner": {
                "type": "string",
                "enum": ["A", "B"],
                "description": "A or B indicating which response is better"
            }
        },
        "required": ["winner"],
        "additionalProperties": False
...
Use the submit_evaluation tool with verdict="pass" if helpful, or verdict="fail" if not helpful."""
...
Use the submit_evaluation tool with verdict="pass" if accurate, or verdict="fail" if inaccurate."""
...
Question: {question}
<response_{first_label}>{first}</response_{first_label}>
<response_{second_label}>{second}</response_{second_label}>

Use the submit_comparison tool to indicate which response is better: '{first_label}' or '{second_label}'."""
        ...
        for item in result.output:
            if item.type == "function_call" and item.name == "submit_comparison":
                args = json.loads(item.arguments)
                return args.get("winner", first_label)
        return first_label
