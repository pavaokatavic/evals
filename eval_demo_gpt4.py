...
Using OpenAI GPT-4o-mini with Chat Completions API

PROS:
- Simple, familiar API (Chat Completions)
- Cost-effective model
- No rate limit issues like Gemini free tier

CONS:
- Text parsing still fragile (same as Gemini)
- Position bias observed (returns "tie" due to inconsistent answers)
- No strict schema enforcement without function calling
"""
...
Reply with exactly one word: 'pass' if helpful, 'fail' if not helpful."""
    ...
    # Fragile parsing - relies on text matching
    return "pass" if "pass" in result.choices[0].message.content.lower() else "fail"
...
Reply with exactly: '{first_label}' or '{second_label}'"""
        ...
        return first_label if first_label in result.choices[0].message.content else second_label

    result1 = run_comparison(resp_a, resp_b, "A", "B")
    result2 = run_comparison(resp_b, resp_a, "B", "A")

    # Often returns "tie" due to position bias in the model
